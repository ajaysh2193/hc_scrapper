...
class UttarakhandHcSpider(scrapy.Spider):

	years = range(2018, 1949, -1)
	case_no = range(1,100000)
	case_type = ["AO","ARBAP","BAC","CAPS","CELE","CEXA","CEXRE","CLCON","CLR","C24","CTR","COMA","COMPA","COMP","CONS", "CONTA",
				 "CRCON","CRJA","CRJR","C482" ,"CRMC", "CRLR", "C407","CROSA","CROSS", "DAO","DCRLA","DCRLR","DSPA","ELEP","EXED",
				 "BA5","FA","FAD", "BA1","BA4","GTA","GTR","GOLD","GA","HABC","ITA","ITAPL","ITR","LA","LIQ","LCRLR","LODG","LSA",
				 "MATA","MCRC","OS","REF","RENC","REVP","STR","SA","BA2","BA7","BAST","BA6","SPA","SPLA","TEST","BA3","DEEP","TTR",
				 "TUA", "C378", "VATR", "WELC", "WTA", "WTAX", "WTR", "WPCRL", "WPMB", "WPMS", "WPPIL", "WPSB", "WPSS"]
	
	name = 'uttarakhand_hc'
	allowed_domains = ['courtnic.nic.in']
	start_urls = ["http://courtnic.nic.in/hcs/content_v1.asp"]
	count = 0
		...
		for year in self.years:
			for case_types in self.case_type:
				for case_nos in self.case_no:
					...
					yield scrapy.Request(url='http://courtnic.nic.in/HCS/list_new2_v1.asp?ctype=' + str(case_types) + "&cno="+ str(case_nos) + "&Cyear="+ str(year) + "&Court_Id=9", callback=self.getCaseDetails, meta={'entity_id': entity_id})
	...
	def getCaseDetails(self, response):
		...
		if (len(error_500) != 0) or (len(error_404) != 0):
			...
			# jsonu = json.dumps({"entity_id": entity_id, "entity_type": "case", "source": "court.nic.in/uttarakhand", "data": item, "processed" : False})
