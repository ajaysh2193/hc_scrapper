...
class CalHcOrginalSpider(scrapy.Spider):

	years = range(2018, 1949, -1)
	case_no = range(1,100000)
	
	case_type = ["AS","AID","APDT","APD","APO","ALP.ITL","ALPITL","AGL","ASCI","AFP","PLA","ASM",
				 "BBA","ABWA","CEXA","ACR","ALP","ALPII","CUSTA","FERA","GCAL","AGWA","AIDR","AIFC",
				 "AOT","SALT","ACRP","AGA","ACWA","ASFC","ATM","AUDA","ACA","ICA","ATA","AIRB","AP",
				 "AA","AC","BIFR","CHS","WPCR","CSOS","CS(O)","CS","CPA","CAA","CA","CP","CC","CRA",
				 "CRCC","EP","AED","EC","AET","EOS","FEA","GA","AGT","AHC","ITA","ITAT","ITP","ITR",
				 "IC","IP","LPA","LM","MS","RVWO","MM","NM","OTS","OCO","OCOT","PMS","ACO","REF","SANA",
				 "SESS","SCO","SS","T", "APOT", "TA","TS","TMA", "TUA", "TBCS", "TCS", "DM", "PLR", 
				 "AWT", "WP.CT"," WP.ST", "WP.TT", "WP"]
		...
		for year in self.years:
			for case_types in self.case_type:
				for case_nos in self.case_no:
					...
					yield scrapy.Request(url='http://courtnic.nic.in/HCS/list_new2_v1.asp?ctype=' + str(case_types) + "&cno="+ str(case_nos) + "&Cyear="+ str(year) + "&Court_Id=1", callback=self.getCaseDetails, meta={'entity_id': entity_id})
	...
	def getCaseDetails(self, response):
		...
		if (len(error_500) != 0) or (len(error_404) != 0):
			...
			# jsonu = json.dumps({"entity_id": entity_id, "entity_type": "case", "source": "court.nic.in/calcutta-original", "data": item, "processed" : False})
