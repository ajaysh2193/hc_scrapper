...
class MaduraiHcSpider(scrapy.Spider):
	name = 'madurai_hc'
	...
	case_type = ["CMSA","CMA","CMA(MD)","CMP","CMSA(MD)","CMP(MD)","CRP NPD","CRP NPD(MD)","CRP PD","CRP PD(MD)",
				 "CRP,CRP(MD)","CONT A","CONT A(MD)","CONT P","CONT P(MD)","CRL A","CRL A(MD)","CRL MP",
				 "CRL MP(MD)","CRL OP","CRL OP(MD)","CRL RC","CRL RC(MD)","CROS.OBJ","CROS.OBJ(MD)","AS",
				 "AS B","AS B(MD)","AS S","AS S(MD)","AS(MD)","HCMP","HCMP(MD)","HCP","HCP(MD)","LPA","LPA(MD)",
				 "MC","MC(MD)","MP","MP(MD)","RT","RT(MD)","REV.APPL","REV.APLC","REV.APLC(MD)","REV.APCR(MD)",
				 "REV.APLW","REV.APLW(MD)","REV.APPL(MD)","REV.STP","REV.STP(MD)","SA","SA(MD)","SUB A",
				 "SUB A(MD)","TC","TCA(MD)","TCP","TCP(MD)","TCR","TCR(MD)","TC(MD)","TCMP","TCMP(MD)","TMA","TMA(MD)",
				 "TMSA","TMSA(MD)","TR CMA","TR CMA(MD)","TR CMP","TR CMP(MD)","TR AS","TR AS(MD)","WAMP","WAMP(MD)",
				 "WPMP","WPMP(MD)","WA","WA(MD)","WMP","WMP(MD)","WP","WP(MD)"]
		...
		for year in self.years:
			for case_types in self.case_type:
				for case_nos in self.case_no:
					...
					yield scrapy.Request(url='http://courtnic.nic.in/HCS/list_new2_v1.asp?ctype=' + str(case_types) + "&cno="+ str(case_nos) + "&Cyear="+ str(year) + "&Court_Id=4", callback=self.getCaseDetails, meta={'entity_id': entity_id})
	...
	def getCaseDetails(self, response):
		...
		if (len(error_500) != 0) or (len(error_404) != 0):
			...
			# jsonu = json.dumps({"entity_id": entity_id, "entity_type": "case", "source": "court.nic.in/madurai-bench", "data": item, "processed" : False})
